...
//        B页面position的取值[1,0]
        ...
        int pageWidth = view.getWidth();//屏幕宽
        ...
        if (position < -1) { // [-Infinity,-1)
            ...
            view.setAlpha(0);//将最左边的界面设置不可见

        } else if (position <= 0) { // [-1,0]
            //        A页面position的取值[0,-1]
            // Use the default slide transition when moving to the left page
            view.setAlpha(1);//设置A页面完全可见
            view.setTranslationX(0);//设置A页面在x轴不移动(为什么A页面移动了？viewpager自带的效果)
            view.setScaleX(1);//设置A页面不缩放
            view.setScaleY(1);
